...
if [[ "$GITLOCATIONCHECK" == *"pingidentity-solutions-c360.git"* ]]; then
    ...
    if [[ "$GITREMOTENAME" == "gh_location" ]]; then
        ...
        echo "$GITREMOTENAME"
        git remote add $GITREMOTENAME "https://${GITHUB_USER}:${GITHUB_TOKEN}@github.com/pingidentity/pingidentity-solutions-c360.git"
        git fetch --unshallow
        git push $GITREMOTENAME HEAD:main
    else
        GITREMOTENAME="gh_location"
